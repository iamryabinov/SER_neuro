...
class RavdessDataset(IemocapDataset):
    ...
    def get_egemaps(self, file_name):
        """
        Todo: egemaps for RAVDESS
        """
        return NotImplementedError

    def get_labels(self, file_name):
        """
        Todo: labels for ravdess. Could use filename parsing, for valence-action-dominance it is suitable to use mapping
        """
        return NotImplementedError

    def get_parsed_dict(self, path):
        """
        Todo: parsed dict for ravdess. Could be None, but need to check references
        """
        return NotImplementedError
