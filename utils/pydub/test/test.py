...
if sys.version_info >= (3, 6):
    class PathLikeObjectTests(unittest.TestCase):
        ...
        def test_non_existant_pathlib_path(self):
            from pathlib import Path
            path = Path('this/pickle_path/should/not/exist/do/not/make/this/exist')
            ...
            # here, just that reading a file from an empty pickle_path is an error.
            with self.assertRaises(OSError):
                _ = AudioSegment.from_file(path)

        def test_non_existant_path_like_str(self):
            path = self.MyPathLike('this/pickle_path/should/not/exist/do/not/make/this/exist')
            with self.assertRaises(FileNotFoundError):
                _ = AudioSegment.from_file(path)
        ...
        def test_non_existant_path_like_bytes(self):
            path = self.MyPathLike(bytes('this/pickle_path/should/not/exist/do/not/make/this/exist', sys.getdefaultencoding()))
            with self.assertRaises(FileNotFoundError):
                _ = AudioSegment.from_file(path)
...
class NoConverterTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        AudioSegment.converter = "definitely-not-a-pickle_path-to-anything-asdjklqwop"
