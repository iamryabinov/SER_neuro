...
class AlexNetEgemaps1792(nn.Module):
    def __init__(self, num_classes=4):
        super(AlexNetEgemaps1792, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=11, stride=4, padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2),
        ...
        self.spec_fc = nn.Sequential(
            nn.Dropout(),
            nn.Linear(256 * 5 * 5, 1792),
            nn.ReLU(inplace=True),
        )

        self.egemaps_fc = nn.Sequential(
            nn.Linear(88, 256),
            nn.ReLU(inplace=True),
        )

        # self.norm = nn.LayerNorm(normalized_shape=2048 + 88)
